...
class LandingPage{
    ...
    get dahLogin() { return $('a[title="Akun"]') }
    get dropdownAkun() { return $('a[class="menu-item"]') }

    get Pants() { return $('[data-label="l2_m_featuredcategory_shorts"]') }
    get dropdownPants() { return $('//div[@class="fr-accordion-head"]//span[@class="fr-accordion-head-text" and text()="BAWAHAN"]/ancestor::button') }
    get CasualPants() { return $('//div[@class="fr-accordion-items" and @role="tabpanel" and contains(@style, "display: block;")]//a[@data-label="/men/bottoms/casual-pants"]') }
    get btnPantWish() { return $('//a[@data-label="E463991-000"]//div[contains(@class, "favorite large swiper-no-swiping")]//button[@aria-label="Favorite"]') }
    get cardLabel() { return $('a[data-label="E463991-000"]') }
    get UserPantWish() { return $('//a[contains(text(), "Wishlist")]') }
    get buktiWish() { return $('a[href="/id/id/products/E463991-000?colorCode=COL57"]') }
    ...
    async getUserPants(){
        await this.Pants.scrollIntoView()
        await this.Pants.waitForClickable();
        await this.Pants.click()
    }
    
    async getdropdownPants(){
        await this.dropdownPants.waitForDisplayed({ timeout: 10000 });
        await this.dropdownPants.scrollIntoView()
        await this.dropdownPants.click()
       }
    
    async getCasualPants(){
        await this.CasualPants.waitForDisplayed({ timeout: 10000 });
        await this.CasualPants.scrollIntoView()
        await this.CasualPants.waitForClickable({ timeout: 10000 });
        await this.CasualPants.click()
       }

    async getPantsWish(){
        await this.btnPantWish.scrollIntoView()
        await this.btnPantWish.waitForClickable();
        await this.btnPantWish.click()
    }

    async getUserWish(){
        await this.UserPantWish.scrollIntoView()
        await this.UserPantWish.waitForClickable();
        await this.UserPantWish.click()
    }

    async getbtnwishLabel(){
        const btnLabel = await this.buktiWish.getAttribute('href')
        return btnLabel
    }

    async getCardLabel(){
        const dataLabel = await this.cardLabel.getAttribute('data-label')
        return dataLabel
    }
